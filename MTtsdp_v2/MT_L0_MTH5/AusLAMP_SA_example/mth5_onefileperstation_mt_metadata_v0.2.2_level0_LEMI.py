...
# Check if survey and survey stations are provided
if len(sys.argv) < 3:
    print("Usage: python your_script.py <SURVEY> <SURVEY_STATIONS>")
    sys.exit(1)

survey = sys.argv[1]

# parse the stations string into a list
stations_str = sys.argv[2]

try:
    stations_all = ast.literal_eval(stations_str)
except (ValueError, SyntaxError):
    print("Error parsing SURVEY_STATIONS. Make sure it is a valid list string.")
    sys.exit(1)
...
work_dir = f'/g/data/<xy12>/<abc123>/AusLAMP/South_Australia/{survey}/level_0/Concatenated_Time_Series_ASCII_per_run'  ### Directory with L0 merged per run ASCII files
mth5_output_directory = f'/g/data/<xy12>/<abc123>/AusLAMP/South_Australia/{survey}/level_0/Concatenated_Time_Series_MTH5_per_run'
full_path_to_mth5_files = sorted(glob.glob(mth5_output_directory+"/*")) 
full_path_to_ascii_files = sorted(glob.glob(work_dir+"/*/*"))
mt_metadata_dir = f'/g/data/<xy12>/<abc123>/supplementing_files/json_metadata/{survey}/export' ### Directory with exported JSON files
...
### define coefficient filters for Earth Data Logger processing. Not required for LEMI stations. 
...
### extracts electromagnetic time series from ASCII generated files
    ...
    with open(EX[0], 'r') as file:
        ...
        try:
            ...
            ### For LEMI loggers, need to convert to float32 first, then to int32
            ex_ts = np.array(EX1).astype(np.float32)
            ex_ts = ex_ts.astype(np.int32)
    with open(EY[0], 'r') as file:
        EY1 = file.read().splitlines()
        try:
            ey_ts = np.array(EY1).astype(np.int32)
        except ValueError:
            ey_ts = np.array(EY1).astype(np.float32)
            ey_ts = ey_ts.astype(np.int32)
    with open(BX[0], 'r') as file:
        BX1 = file.read().splitlines()
        try:
            bx_ts = np.array(BX1).astype(np.int32)
        except ValueError:
            bx_ts = np.array(BX1).astype(np.float32)
            bx_ts = bx_ts.astype(np.int32)
    with open(BY[0], 'r') as file:
        BY1 = file.read().splitlines()
        try:
            by_ts = np.array(BY1).astype(np.int32)
        except ValueError:
            by_ts = np.array(BY1).astype(np.float32)
            by_ts = by_ts.astype(np.int32) 
    with open(BZ[0], 'r') as file:
        BZ1 = file.read().splitlines()
        try:
            bz_ts = np.array(BZ1).astype(np.int32)
        except ValueError:
            bz_ts = np.array(BZ1).astype(np.float32)
            bz_ts = bz_ts.astype(np.int32)
    return ex_ts, ey_ts, bx_ts, by_ts, bz_ts


# Function to extract the numeric part after 'SA81-' and before '_'
def get_run_number(filename):
    # Example filename: 'SA81-001_012.BX'
    try:
        base_name = filename.split('.')[0]  # 'SA81-001_012'
        parts = base_name.split('-')        # ['SA81', '001_012']
        if len(parts) > 1:
            number_part = parts[1]          # '001_012'
            # The run number is the first part before '_'
            run_number_str = number_part.split('_')[0]  # '001'
...
### creates Level 0 mth5 file with associated mt_metadata
    ...
    add_station = m.add_station(station, survey=survey)
    ...
    ### for stations with a single run, extract run, ex, ey, bx, by, bz metadata from associated station metadata JSON files
        ...
        add_run = m.add_run(station, run_number, survey=survey) 
        ...
        ex = m.add_channel(station, run_number, "ex", "electric", ex_ts, survey=survey)
        ey = m.add_channel(station, run_number, "ey", "electric", ey_ts, survey=survey)
        bx = m.add_channel(station, run_number, "bx", "magnetic", bx_ts, survey=survey)
        by = m.add_channel(station, run_number, "by", "magnetic", by_ts, survey=survey)
        bz = m.add_channel(station, run_number, "bz", "magnetic", bz_ts, survey=survey)
        ...
        # above).                                                                             #
        #                                                                                     #
        # ex = m.add_channel(station, run_number, "ex", "electric", None, survey=survey)      #
        # ey = m.add_channel(station, run_number, "ey", "electric", None, survey=survey)      #
        # bx = m.add_channel(station, run_number, "bx", "magnetic", None, survey=survey)      #
        # by = m.add_channel(station, run_number, "by", "magnetic", None, survey=survey)      #
        # bz = m.add_channel(station, run_number, "bz", "magnetic", None, survey=survey)      #
        #                                                                                     #
        ...
        #print('split_lists_original_start =', split_lists[0])
        if len(split_lists) == 2:
            run_num_list0 = get_run_number(split_lists[0][0])
            run_num_list1 = get_run_number(split_lists[1][0])
            if (run_num_list0 is not None and run_num_list1 is not None):
                if (run_num_list0 <= 3 and run_num_list1 >= 300):
                    split_lists[0], split_lists[1] = split_lists[1], split_lists[0]

        #print('split_lists_new_start =', split_lists[0])
        ...
        for i, (group,mt_meta) in enumerate(zip(split_lists,mt_metadata_files)):
            ...
            add_run = m.add_run(station, run, survey=survey)            
            add_run.metadata.from_dict(run_dict)
            add_run.write_metadata()

            ex_ts,ey_ts,bx_ts,by_ts,bz_ts = channel_data_extraction(group)

            ex = m.add_channel(station, run, "ex", "electric", ex_ts, survey=survey)
            ey = m.add_channel(station, run, "ey", "electric", ey_ts, survey=survey)
            bx = m.add_channel(station, run, "bx", "magnetic", bx_ts, survey=survey)
            by = m.add_channel(station, run, "by", "magnetic", by_ts, survey=survey)
            bz = m.add_channel(station, run, "bz", "magnetic", bz_ts, survey=survey)
