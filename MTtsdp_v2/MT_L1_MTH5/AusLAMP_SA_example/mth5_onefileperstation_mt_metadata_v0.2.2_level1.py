...
if len(sys.argv) < 4:
    print("Usage: python your_script.py <SURVEY> <STATIONS_SINGLE_RUN> <STATIONS_MULTIPLE_RUNS>")
    sys.exit(1)
...
stations_single_run = ast.literal_eval(sys.argv[2])

stations_multiple_runs = ast.literal_eval(sys.argv[3])
...
work_dir = f'/g/data/xr05/nre900/AusLAMP/South_Australia/{survey}/level_0/Concatenated_Time_Series_MTH5_per_run'  ### directory with the L0 MTH5 files
downsampled_outdir = f'/g/data/xr05/nre900/AusLAMP/South_Australia/{survey}/level_1/Concatenated_Resampled_Rotated_Time_Series_MTH5'
full_path_to_mth5_files = sorted(glob.glob(work_dir+"/*")) 
full_path_to_downsampled_outdir = sorted(glob.glob(downsampled_outdir+"/*"))
mt_metadata_downsampled_dir = f'/g/data/xr05/nre900/supplementing_files/json_metadata_downsampled/{survey}/export'
...
def channel_data_extraction_rotation_downsampling_unit_conversion(mth5_file,station,run,resampling_rate):
    ...
    # if dipole lengths weren't recorded in metadata, assume 50 m dipole lengths as this was the default for AusLAMP SA.
    if str(xlength).lower() == 'none':
        xlength = 50.0
    if str(ylength).lower() == 'none':
        ylength = 50.0
    ...
    print('decimated ey dtype:', decimate(ey, resample_factor).astype(float).dtype)
    print('ylength:', ylength, type(ylength))
    print('gainE:', gainE, type(gainE))
    print('gainEonly:', gainEonly, type(gainEonly))

    intermediate_ex = decimate(ex, resample_factor).astype(float) / float(xlength) * float(gainE) / float(gainEonly)
    intermediate_ey = decimate(ey, resample_factor).astype(float) / float(ylength) * float(gainE) / float(gainEonly)
...
for i,station in enumerate(sorted(stations_single_run)):
    ...
    #print('station_l0_timeseries:', station_l0_timeseries)
